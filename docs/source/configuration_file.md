...
#### contrasts
- `train_validation`: list of image modalities loaded for the training and validation. If `multichannel=True`, this list represents the different channels of the input tensors. Otherwise, the modalities are mixed and the model only has one input channel.
