...
The Monte Carlo samples are then used to compute uncertainties measures for each image. The results are saved under
``pred_masks``.
...
Six files are generated during this process for each testing image:4
...
- ``*_unc-cv.nii.gz``: Structuve-wise measure of uncertainty derived from the coefficient of variation of the volume
