...
def extract_mid_slice_and_convert_coordinates_to_heatmaps(path, suffix, aim=-1):
    ...
    """
    t = [d.name for d in Path(path).iterdir() if d.name != 'derivatives']

    for i in range(len(t)):
        sub = t[i]
        path_image = Path(path, t[i], 'anat', t[i] + suffix + '.nii.gz')
        if path_image.is_file():
            path_label = Path(path, 'derivatives', 'labels', t[i], 'anat', t[i] + suffix +
            ...
            nib.save(mid_nifti, Path(path, t[i], 'anat', t[i] + suffix + '_mid.nii.gz'))
            ...
            nib.save(nib_pred, Path(path, 'derivatives', 'labels', t[i], 'anat', t[i] + suffix +
