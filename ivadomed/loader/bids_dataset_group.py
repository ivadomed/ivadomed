...
class FileDatasetGroup:
    ...
    def __init__(self,
                 dict_bids_group_spec: dict,
                 config: GeneralizedLoaderConfiguration
                 ):
        """
        Constructor that leverage a generalized loader configuration
        Args:
            dict_bids_group_spec: An example shown above in example_json dict.
            config:
        """
        print(dict_bids_group_spec)
        ...
        self.path_data = dict_bids_group_spec.get(DataloaderKW.PATH_DATA, ".")
        ...
        self.n_expected_input: int = int(dict_bids_group_spec.get(DataloaderKW.EXPECTED_INPUT, 0))
        if self.n_expected_input <= 0:
            raise ValueError("At DataSetGroup level, Number of Expected Input must be > 0")

        self.n_expected_gt: int = int(dict_bids_group_spec.get(DataloaderKW.EXPECTED_GT, 0))
        ...
        if (ModelParamsKW.IS_2D not in config.model_params and not config.model_params.get(ModelParamsKW.IS_2D)):
            self.validate_update_3d_train_val_test_and_filename_pairs(config, dict_bids_group_spec)
        else:
            self.validate_update_2d_train_val_test_dataset_and_filename_pairs(dict_bids_group_spec)

        # Assign default labels if not provided.
        self.name: str = dict_bids_group_spec.get(DataloaderKW.DATASET_GROUP_LABEL, "DefaultDatasetGroupLabel")
