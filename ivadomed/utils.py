...
class SliceFilter(mt_filters.SliceFilter):
    ...
    slices that:
         -  has only zeros after center cropping. To
         avoid feeding empty inputs into the network.
         - has only few 1-voxels in the ground-truth,
         which could vanish during resampling.
    """
    def __init__(self, filter_empty_mask=True,
                 filter_empty_input=True, nb_nonzero_thr=10):
        super().__init__(filter_empty_mask, filter_empty_input)
        self.nb_nonzero_thr = nb_nonzero_thr
