import ivadomed.architecture.blocks
...
def test_model_3d_att():
    ...
    inp = torch.tensor(a).float()
    model = ivadomed.architecture.unet3d.Modified3DUNet(in_channel=1, out_channel=1, attention=True)
    inf = model(inp)
    assert(type(inf) == torch.Tensor)
...
def test_filmed_unet():
    a = [[[[0 for i in range(100)] for i in range(100)]]]
    inp = torch.tensor(a).float()
    model = ivadomed.architecture.unet.FiLMedUnet()
    inf = model(inp)
    assert (type(inf) == torch.Tensor)
...
def test_film_generator():
    ...
    inp = torch.tensor(a).float()
    model = ivadomed.architecture.blocks.FiLMgenerator(64, 1)
    inf = model(inp)
