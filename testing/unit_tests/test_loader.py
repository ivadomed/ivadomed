...
from testing.common_testing_util import remove_tmp_dir, bids_dataframe_comparison_framework, prepare_loader_dataset
...
def test_bids_df_microscopy_png(download_data_testing_test_files, loader_parameters, target_csv):
    """
    ...
    bids_dataframe_comparison_framework(loader_parameters, __tmp_dir__, target_csv)


@parametrize_with_cases("loader_parameters, target_csv", cases=[
    case_bids_df_anat,
])
def test_bids_df_anat(download_data_testing_test_files, loader_parameters, target_csv):
    """
...
@parametrize_with_cases("loader_parameters, target_csv, test_csv", cases=[
    case_bids_df_multi
])
def test_bids_df_multi(download_data_testing_test_files, loader_parameters, target_csv, test_csv):
    """
    Test for multiple folders in path_data
    """

    bids_dataframe_comparison_framework(loader_parameters, __tmp_dir__, target_csv, test_csv)


@parametrize_with_cases("loader_parameters, target_csv", cases=[
    case_bids_df_ctscan,
])
def test_bids_df_ctscan(download_data_testing_test_files, loader_parameters, target_csv):
    """
...
def test_2d_patches_and_resampling(download_data_testing_test_files,
    ...
    """
    ds = prepare_loader_dataset(loader_parameters, model_parameters, transform_parameters, __tmp_dir__)
...
def test_get_target_filename_list(loader_parameters, model_parameters, transform_parameters):
    ...
    """
    test_ds = prepare_loader_dataset(loader_parameters, model_parameters, transform_parameters, __tmp_dir__)
