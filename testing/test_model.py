...
def test_modified_decoder():
    ...
    a = [torch.zeros((1, 64, 322, 322)), torch.zeros((1, 128, 161, 161)),
         torch.zeros((1, 256, 80, 80)), torch.zeros(
            (1, 512, 40, 40))]  # by changing the depth, the length of the input feature list should be changed.
...
def test_unet_att2d():
    ...
    a = [[[[0 for i in range(322)] for i in range(322)]]]
    inp = torch.tensor(a).float()
    model = imed_model.Unet(in_channel=1, out_channel=1, depth=3, drop_rate=0.4, bn_momentum=0.1,
